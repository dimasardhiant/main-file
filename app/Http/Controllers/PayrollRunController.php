    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-payroll-runs')) {
            ...
            if ($request->has('branch') && !empty($request->branch) && $request->branch !== 'all') {
                $query->whereHas('payrollEntries.employee.employee', function ($q) use ($request) {
            ...
            return Inertia::render('hr/payroll-runs/index', [
                ...
                'filters' => $request->all(['search', 'status', 'date_from', 'date_to', 'branch', 'department', 'designation', 'sort_field', 'sort_direction', 'per_page']),
            ]);
        } else {
